...
## 🎥 映像そのものの解析(本命ルート、要セットアップ)
**ユーザーの本当の目的は「画面で何を操作しているか」**(文字起こしでは足りない)。これには**Gemini APIを「目」として使う**:
```
python3 scripts/yt_video_analyze.py <YouTube URL> [着目点]
```
Gemini がYouTube URLを直接受け取り映像+音声を視聴 → 画面上の操作(コマンド原文・ファイル編集・UI操作・結果)を時系列で書き出す → `inbox/videos/*-visual.txt` に保存 → **video-analyzer**エージェントで構造化 → `research/videos/` → knowledge/lab反映。
**必要(ユーザーの一回きり設定)**: ①`GEMINI_API_KEY`(https://aistudio.google.com/apikey で無料発行)を環境変数に ②ネットワーク許可に `generativelanguage.googleapis.com` を追加。
コスト: 無料枠で1日数本は解析可能(超過時も動画1本数円〜数十円程度)。

## その他の自動化(条件が整えば)
- YouTube Data APIキー(`YOUTUBE_API_KEY`)+googleapis.com許可 → 概要欄・統計の自動取得
- 文字起こしコピペ方式(従来ルート)も併用可(字幕自動取得はYouTubeのbot検知で不可と実測済み)
