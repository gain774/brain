...
### ⚡ 超軽量ルール(最優先): 重い抽出は1日1回だけ
その日すでに `research/YYYY/YYYY-MM-DD.md` に「実践情報」まとめが書かれているか確認する。**書かれていれば、この回は手順2(収集)+生データ保存+コミットだけで終了**(手順4-5の重い抽出・整理はしない)。まだなら手順4以降を実施(=1日の最初の回だけ重い処理)。これで枠消費を最小化する。

### 4. 日次ログ記録 — 役割特化サブエージェントで分担(薄いオーケストレーター方式)
**重要(コスト最適化, K-A8/A9)**: メインセッションは永続で累積コンテキストが巨大=高コスト。重い処理は`.claude/agents/`の**役割特化サブエージェント(全てSonnet・安価)**に分担させ、メインは指示・貼り付け・コミットに徹する。
- 抽出・評価は **`signal-extractor`** エージェントに委譲 → 「日次ログにそのまま貼れる評価スコア付きmarkdown」が返るので**メインは貼るだけ**
- 件数・カテゴリが多ければ**カテゴリ別に複数を1メッセージで並列起動**(background並列がデフォルト)
- `research/YYYY/YYYY-MM-DD.md` に追記(同日2回目以降は `## 実行 HH:MM`)

### 5. 検証・監査・実測(役割分担)
- **fact-checker** エージェント: 総合8以上/疑義フラグの主張を一次情報で裏取り(WebSearch)
- **knowledge-auditor** エージェント: knowledgeの鮮度・未確定・出典・重複を機械監査(要再検証項目を洗い出す)
- **adversarial-reviewer** エージェント: 週次(日曜)またはknowledgeが育った時、全体を敵対的レビュー
- `lab/queue.md` から1〜3件(夜間は全件)を**実際に動かして**検証、`lab/YYYY-MM-DD-*.md` に記録、判定(✅/⚠️/❌)をknowledgeへ反映
※これらエージェント定義は `.claude/agents/*.md`。呼べない環境では `Agent(subagent_type: general-purpose, model: "sonnet")` に同等の役割プロンプトを渡す。
...
## エラー時の自己修正(止まらない)
一時エラー(API/接続/サブエージェント失敗)は1回自動リトライ。自作スクリプトのバグはその場で修正→テスト→続行。2回続く同種エラーは原因調査し恒久修正+loop記録。外部要因はスキップして残りを続行。詳細はBRAIN.md「エラー自己修正の原則」。
