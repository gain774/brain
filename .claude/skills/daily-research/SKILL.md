...
### 4. 日次ログ記録 — 「薄いオーケストレーター+安い並列ワーカー」方式
**重要(コスト最適化, K-A8/A9)**: このメインセッションは永続で累積コンテキストが巨大=高コスト。重い読み込み・評価は**毎回まっさらで安いサブエージェントに逃がし**、メインは指示とファイル書き込みとコミットに徹する。具体:
- 生データの抽出・評価は **`Agent(subagent_type: general-purpose, model: "sonnet")`** に委譲(Sonnetで十分・Opusの半額以下。2026-07-16実測で有効性確認)
- サブエージェントには「**日次ログにそのまま貼れる、評価スコア(N/K/M/P型+10段階)付きの完成markdown**」まで作らせ、メインは貼るだけにする(メインの思考=高コストを最小化)
- 件数が多い/カテゴリが多いときは**カテゴリ別に複数サブエージェントを1メッセージで並列起動**(公式でbackground並列がデフォルト)
- **知名度でなく内容で選別**するようサブエージェントに明示

`research/YYYY/YYYY-MM-DD.md` に追記(同日2回目以降は `## 実行 HH:MM`)。

### 5. 実測検証(lab)
`lab/queue.md` から1〜3件(夜間で余裕あれば全件)を**実際に動かして**検証、`lab/YYYY-MM-DD-*.md` に記録、判定(✅/⚠️/❌)を knowledge へ反映。knowledge鮮度監査・重複チェック等の機械的タスクも**Sonnetサブエージェント**に委譲するとメインを汚さず安い。
